...
export const setUserTemp = ({ email, username, token }) => ({
  type: SET_USER_TEMP,
  payload: {
    email,
    username,
...
const registerEpic = (action$, state$) => {
  return action$.pipe(
    ...
    mergeMap(([action, state]) => {
      const { email, password, birth } = state.auth.form;
      return ajax.post(`/api/auth/register/`, { email, password, birth }).pipe(
        map(response => {
...
const loginEpic = (action$, state$) => {
  return action$.pipe(
    ...
    mergeMap(([action, state]) => {
      ...
      return ajax.post(`/api/auth/login/`, { email, password }).pipe(
        map(response => {
          const { user, token } = response.response;
